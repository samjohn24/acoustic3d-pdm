...
jtag_bytestream = jtag.BytestreamClient(queue, jtag_buf_size=48*10, buf_elem_size=48*100, max_misses=10000)
...
#for i in range(10):
    ...
    #data_int = np.array(jtag_bytestream.getData(True))
    data_int = np.array(jtag_bytestream.getData())
    print "received %d samples:" %(len(data_int)),data_int
